...
2. **Variables de entorno** (Project Settings → Environment Variables) — cargar una por una, nunca como archivo `.env` subido:
   - `DATABASE_URL` — **tiene que ser el Transaction pooler de Supabase** (Project Settings → Database → Connection string → Transaction pooler, puerto 6543), **no** el Session pooler ni la conexión directa. Vercel corre cada ruta como una función serverless: bajo tráfico concurrente pueden convivir varias invocaciones en paralelo, cada una con su propio pool de conexiones. El Transaction pooler (Supavisor/PgBouncer) está diseñado justo para eso — multiplexa muchas conexiones de cliente sobre pocas conexiones reales a Postgres. Usar el Session pooler acá agota rápido el límite de conexiones y produce `EMAXCONNSESSION` en los logs de runtime. El Session pooler (puerto 5432) queda reservado para `prisma migrate deploy` corrido a mano desde tu máquina (ver paso 5) — nunca para `DATABASE_URL` de producción.
...
5. **Migraciones**: `npx prisma migrate deploy` **no** corre automáticamente en el build de Vercel — hay que aplicarlas a mano contra la base de staging/producción (una vez, o cada vez que se agreguen migraciones nuevas) desde tu máquina o CI, con `DATABASE_URL` apuntando temporalmente al **Session pooler** (soporta prepared statements; el Transaction pooler configurado en Vercel no). No reutilizar esa cadena de conexión como `DATABASE_URL` de Vercel.
