...
export default async function ClubDetallePage({
  ...
  return (
    <div className="flex flex-1 flex-col gap-4">
      ...
      <Link
        href={`/admin/clubes/${club.id}/jugadores`}
      ...
      <div className="grid grid-cols-3 gap-3 sm:w-80">
        <Link
          href={`/admin/clubes/${club.id}/jugadores`}
          className="rounded-lg border border-border bg-surface p-3 text-center transition-colors hover:border-accent-blue/40"
        >
          <div className="text-lg font-semibold text-foreground">{club._count.jugadores}</div>
          <div className="text-xs text-muted underline">Jugadores</div>
        </Link>
